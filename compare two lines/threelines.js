...
let drawCelloLine = false;
let drawViolaLine = false;
let drawViolinLine = true;
...
function preload() {
  ...
  // darkest blue
  loadTable("MATT2 MEDIUM.csv", "csv", "header", function(table) {
  ...
  // saturated blue
  loadTable("MATTEO LAYER 3.csv", "csv", "header", function(table) {
...
// JavaScript to toggle the play/pause state
...
//   // Move all points upwards for all lines
// const yOffset = -100;
// for (let i = 0; i < currentDataPointIndex1; i++) {
//   dataPoints1[i].yPos += yOffset;
//   dataPoints2[i].yPos += yOffset;
//   dataPoints3[i].yPos += yOffset;
...
text("Attempt III - X Position: " + dataPoints3[currentDataPointIndex3].xPos.toFixed(2), 20, height - 60);
text("Attempt III - Y Position: " + dataPoints3[currentDataPointIndex3].yPos.toFixed(2), 20, height - 40);
